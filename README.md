...
> **Stack:** Next.js 16 · TypeScript · Prisma 5 · PostgreSQL · Tailwind CSS v4 · JWT · Upstash Redis · Cloudinary · Konnect · Nodemailer · Twilio
...
| Local dev | Set `DEV_DEFAULT_CLUB_SLUG=demo` in `.env.local` to bypass subdomain requirement |
...
| POST | `/api/cron/trial-check` | CRON_SECRET | Suspend expired trials |
| POST | `/api/cron/subscription-renewal` | CRON_SECRET | Process renewals |
| POST | `/api/cron/session-reminders` | CRON_SECRET | Send session reminder notifications |
...
4. Response: Set-Cookie: token=<JWT>; HttpOnly; SameSite=Strict; Secure
...
**Google OAuth:** `/api/auth/google` → Google → `/api/auth/google/callback` → creates/finds user → same JWT cookie flow.
...
```bash
# Run all 86 existing tests + 4 new Phase 11 test suites
npx vitest run
...
| 12 | Production Readiness | ✅ /api/health + Sentry + vercel.json + .env.example |
...
| 5 | SSE (`/api/sse`) uses in-memory Map — won't scale across multiple instances; needs Redis pub/sub | High (before horizontal scale) |
| 6 | Prisma binary blocked in sandbox — run `prisma generate + migrate` in real env | Infra |
...
# 5. Seed demo data (2 clubs, plans, members, coaches)
npx prisma db seed
...
Access the app at `http://demo.localhost:3000` (requires `DEV_DEFAULT_CLUB_SLUG=demo` in `.env.local` or a local DNS/hosts setup).
...
| `KONNECT_BASE_URL` | ✅ | |
| `KONNECT_WALLET_ID` | ✅ | |
| `SMTP_URL` | ✅ | Email sending |
| `SMTP_FROM` | ✅ | |
| `TWILIO_ACCOUNT_SID` | Optional | SMS |
| `TWILIO_AUTH_TOKEN` | Optional | |
| `TWILIO_FROM_NUMBER` | Optional | |
| `CRON_SECRET` | ✅ | Auth header for Vercel Cron jobs |
| `SENTRY_DSN` | Optional | Error monitoring (server) |
| `NEXT_PUBLIC_SENTRY_DSN` | Optional | Error monitoring (browser) |
| `DEV_DEFAULT_CLUB_SLUG` | Dev only | Default club for local dev without subdomain |
