...
export async function initKonnectPayment(
  ...
  const res = await fetchWithTimeout(`${KONNECT_BASE_URL}/payments/init-payment`, {
    method: "POST",
    headers: {
...
/**
 * Konnect payment refs are opaque ids. This is the shape we accept from the
 * (public, unauthenticated) webhook query string before it goes anywhere near
 * a URL path or a database query.
 */
...
export async function getKonnectPaymentDetails(
  ...
  if (!isValidPaymentRef(paymentRef)) {
    throw new Error("Invalid Konnect payment ref");
  }
  const res = await fetchWithTimeout(`${KONNECT_BASE_URL}/payments/${encodeURIComponent(paymentRef)}`, {
