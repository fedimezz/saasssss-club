...
export function buildAuthCookieOptions(requestUrl?: string) {
  ...
  let cookieDomain = configuredDomain;
  if (!cookieDomain && requestUrl) {
    const hostname = new URL(requestUrl).hostname;
    const isLocalhost = hostname === "localhost" || hostname.endsWith(".localhost") || /^\d{1,3}(\.\d{1,3}){3}$/.test(hostname);
    if (!isLocalhost && hostname.includes(".")) cookieDomain = `.${hostname}`;
  }
