...
const GOOGLE_ERROR_MESSAGES: Record<string, string> = {
  ...
  club_unavailable: "__CLUB_UNAVAILABLE__",
};
  ...
  return (
    <div className={`min-h-screen flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 transition-all duration-300 ${
      ...
      <div className={`max-w-md w-full space-y-8 p-8 rounded-2xl shadow-2xl ${
        ...
        {error && (
          error === "__CLUB_UNAVAILABLE__" ? (
            <div className="bg-amber-50 dark:bg-amber-950/40 border-l-4 border-amber-500 text-amber-700 dark:text-amber-300 px-4 py-3 rounded-lg text-sm space-y-1">
              <p className="font-semibold">Vous êtes sur la plateforme GymOS</p>
              <p>
                Si vous êtes <strong>propriétaire d&apos;un club</strong>,{" "}
                <Link href="/platform/login" className="underline font-semibold hover:text-amber-900 dark:hover:text-amber-100">
                  connectez-vous ici
                </Link>
                .
              </p>
              <p>
                Si vous êtes <strong>membre d&apos;un club</strong>, utilisez l&apos;adresse de votre club (ex.&nbsp;
                <code className="bg-amber-100 dark:bg-amber-900/60 px-1 rounded">monclub.gymos.com</code>
                ).
              </p>
            </div>
          ) : (
