...
function TeamCoachCard({ coach, isDarkMode, delay }: { coach: RealCoach; isDarkMode: boolean; delay: number }) {
  return <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay }} className={`rounded-2xl overflow-hidden shadow-lg ${isDarkMode ? "bg-neutral-900" : "bg-white"}`}><div className="relative h-56">{coach.photoUrl ? <Image src={coach.photoUrl} alt={coach.name} fill sizes="(max-width: 768px) 100vw, 33vw" className="object-cover" /> : <div className={`w-full h-full flex items-center justify-center ${isDarkMode ? "bg-neutral-800" : "bg-gray-100"}`}><Dumbbell size={36} className={isDarkMode ? "text-neutral-600" : "text-gray-300"} /></div>}</div><div className="p-6"><h3 className={`text-xl font-bold mb-1 ${isDarkMode ? "text-white" : "text-gray-900"}`}>{coach.name}</h3>{(coach.specialties.length > 0 || coach.activities.length > 0) && <p className="text-sm font-medium text-green-600 mb-2">{coach.specialties.length > 0 ? coach.specialties.join(" · ") : coach.activities.map((a) => ACTIVITY_LABELS[a] ?? a).join(" · ")}</p>}{coach.bio && <p className={`text-sm leading-relaxed ${isDarkMode ? "text-neutral-400" : "text-gray-600"}`}>{coach.bio}</p>}</div></motion.div>;
}
function CoachCard({ image, title, text, isDarkMode, delay }: { image: string; title: string; text: string; isDarkMode: boolean; delay: number }) {
  return <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay }} className={`rounded-2xl overflow-hidden shadow-lg ${isDarkMode ? "bg-neutral-900" : "bg-white"}`}><div className="relative h-48">{image ? <Image src={image} alt={title} fill sizes="(max-width: 768px) 100vw, 33vw" className="object-cover" /> : <div className={`w-full h-full flex items-center justify-center ${isDarkMode ? "bg-neutral-800" : "bg-gray-100"}`}><Dumbbell size={36} className={isDarkMode ? "text-neutral-600" : "text-gray-300"} /></div>}</div><div className="p-6"><h3 className={`text-xl font-bold mb-2 ${isDarkMode ? "text-white" : "text-gray-900"}`}>{title}</h3><p className={`text-sm leading-relaxed ${isDarkMode ? "text-neutral-400" : "text-gray-600"}`}>{text}</p></div></motion.div>;
}
