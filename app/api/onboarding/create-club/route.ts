...
import { generateToken, generateBridgeToken, hashBridgeNonce } from "@/lib/auth";
import { buildAuthCookieOptions, setAuthCookie } from "@/lib/auth-cookie";
...
const schema = z.object({
  ...
  // Strict on input (lowercase only — the onboarding form slugifies), then the
  // shared lib/slug.ts rules: length, hyphen placement AND the reserved list
  // (admin, billing, mail, support, ... can't be registered as subdomains).
  slug: z
      .string().trim()
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Single-use, purpose-bound token for the /api/auth/bridge handoff to the
    // owner's own subdomain, bound to THIS browser through a nonce cookie —
    // see that route for the full set of checks.
    ...
    setAuthCookie(res, token, request.url, { maxAgeSeconds: 60 * 60 * 24 * 7 });
    // Domain-shared in production (same attributes as the session cookie) so
    // it reaches {slug}.host; the bridge route checks and clears it.
    res.cookies.set(BRIDGE_NONCE_COOKIE, nonce, { ...buildAuthCookieOptions(request.url), maxAge: 120 });
    return res;
  } catch (error) {
