...
export async function PATCH(
  ...
  try {
    ...
    const existing = await prisma.session.findFirst({
      where: { id, clubId: admin.clubId as string },
    ...
    if (coachId) {
      const coachExists = await prisma.coach.findFirst({ where: { id: coachId, clubId: admin.clubId as string }, select: { id: true } });
      if (!coachExists) {
...
export async function DELETE(
  ...
  try {
    ...
    const session = await prisma.session.findFirst({
      where: { id, clubId: auth.user.clubId as string },
      include: {
        userSessions: {
    ...
    if (affectedUserIds.length > 0) {
      ...
      runAfter(() =>
        notifyUsers(clubId, affectedUserIds, {
