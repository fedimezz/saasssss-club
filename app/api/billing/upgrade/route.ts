...
// and logs the action.
//
// THIS ENDPOINT DOES NOT CHARGE ANYTHING. Left open, any club owner could
// switch to any paid plan for free: it marks the subscription ACTIVE and
// clears trialEndsAt, which also takes the club out of the trial-expiry cron.
// So in production it is refused unless ENABLE_MANUAL_PLAN_SWITCH=true is set
// explicitly (staging / demos / until SaaS billing is wired to a provider).
// Non-production environments keep working unchanged.
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (process.env.NODE_ENV === "production" && process.env.ENABLE_MANUAL_PLAN_SWITCH !== "true") {
      return NextResponse.json(
        { error: "Le changement de plan en ligne n'est pas encore disponible. Contactez le support." },
        { status: 403 }
      );
    }
