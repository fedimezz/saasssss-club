...
// After /api/onboarding/create-club succeeds, the owner is already
// authenticated (that route sets the auth cookie itself). Build the URL to
// their new club's own subdomain so we land them there directly instead of
// sending them through a second login step.
function ownerSettingsDestination(slug: string): string {
  const baseUrl = process.env.NEXT_PUBLIC_APP_URL ?? window.location.origin;
  const target = new URL("/admin/settings", baseUrl);
...
export default function OnboardingWizardPage() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      // create-club already logged the owner in (it sets the auth cookie).
      // Send them straight to their new club's settings on its own
      // subdomain — a hard navigation so the cookie rides along — instead
      // of making them log in again through a separate screen.
      window.location.href = ownerSettingsDestination(json.club.slug);
    } catch {
